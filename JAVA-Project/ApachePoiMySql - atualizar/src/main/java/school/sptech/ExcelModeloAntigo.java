...
public class ExcelModeloAntigo extends Excel {
    private List<RegistroAcidente> registros;

    public ExcelModeloAntigo(String arquivoTratar, Integer idPadrao) {
        super(arquivoTratar, new Integer[]{0, 1, 4, 5, 6, 7, 8, 12, 13, 14, 15, 16, 17, 19, 20, 21, 22, 24, 25},idPadrao);
        ...
        try {
            ...
            for (Row row : sheet) {
                ...
                if(registros.size()==1000){
                    comandos.atualizarLote(registros,getArquivoTratar());
                    registros.clear();
                }
            }
            workbook.close();
